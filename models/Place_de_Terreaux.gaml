...
global {
	...
	shape_file init_space_shapefile <- shape_file("../includes/Place de Terreaux/init_space.shp");
	...
	reflex init_ped when: mod(cycle, 5)=0 {
...
species people skills: [pedestrian]{
	...
	reflex move_to_attraction {
		if (final_waypoint = nil) {
			my_target <- any_location_in(center_area);
		...
		if (distance_to(location, my_target) < 5.0) {
			final_waypoint <- nil;
		}
	}	
