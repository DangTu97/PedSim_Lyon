...
model generatepedestrianpaths

/* Insert your model definition here */

global {
	file wall_shapefile <- file("../includes/cleaned_map/polygon2.shp");
	file barrier_shapefile <- file("../includes/Place de Terreaux/barrier.shp");
	file line_shapefile <- file("../includes/Place de Terreaux/lines.shp");
...
//	float min_dist_obstacles_filtering <- 0.0;// minimal distance to obstacles to keep a path (float; if 0.0, no filtering), 
	...
	bool add_points_open_area <- true;//add points to open areas
 	bool random_densification <- false;//random densification (if true, use random points to fill open areas; if false, use uniform points), 
 	float min_dist_open_area <- 0.0;//min distance to considered an area as open area, 
 	float density_open_area <- 0.0; //density of points in the open areas (float)
 	bool clean_network <-  true; 
	float tol_cliping <- 0.0; //tolerance for the cliping in triangulation (float; distance), 
...
//		create wall from:barrier_shapefile {
//			open_area <- open_area -(shape buffer (P_shoulder_length/2.0));
//		}
...
//		list<geometry> generated_lines <- line_shapefile;
